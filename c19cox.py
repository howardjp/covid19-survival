...
logLevelMap = {"trace" : spdlog.LogLevel.TRACE, "debug" : spdlog.LogLevel.DEBUG, "info" : spdlog.LogLevel.INFO,
               "warn" : spdlog.LogLevel.WARN, "err" : spdlog.LogLevel.ERR, "critical" : spdlog.LogLevel.CRITICAL, "off" : spdlog.LogLevel.OFF }

def main():
    """Console script for c19cox."""

    parser = argparse.ArgumentParser()
    parser.add_argument('--device', metavar = "device", type = str, default = None, choices = ['cpu', 'cuda', 'opengl', 'opencl'], help = 'set the processing device for PyTorch')
    parser.add_argument('--data', metavar = "data", type = str, default = "abbrev.json.bz2", help = "dataset to use")
    parser.add_argument('--name', metavar = "name", type = str, default = pathlib.Path(tempfile.mktemp(suffix='', prefix='run-', dir=".")), help = "use name as output file base")
    parser.add_argument('--type', metavar = "type", type = str, default = "coxcc",  choices = ['coxcc', 'coxph', 'pchazard'], help = 'set the model type')
    parser.add_argument('--interp', metavar = "interp", type = str, default = "cubic",  choices = ['cubic', 'linear'], help = 'set the interpolation type')
    parser.add_argument('--loglevel', metavar = "level", type = str, default = "info",  choices = ['trace', 'debug', 'info', 'warn', 'error', 'critical', 'off'], help = 'set the logging level to the console')
    parser.add_argument('--maxepochs', metavar = "count", type = int, default = 200, help = 'set the maximum number of training epochs')
    ...
    synthea.make_model(console, file_name = opts["data"], output_name = opts["name"], model_type = opts["type"], batch_size = opts["batchsize"], max_epochs = opts["maxepochs"], verbose = opts["verbose"], interpolation = opts["interp"])
